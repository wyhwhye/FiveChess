...
public class FiveChessFrameU extends JFrame implements MouseListener {
    ...
    BufferedImage bgimgWhite = null;  // 白色背景
    ...
    public FiveChessFrameU() throws IOException {
    ...
    public void paint(Graphics g2){
        ...
        if (finish == true){ // 游戏结束
            if (chessCount == 361){  // 棋盘下满
                g.drawString( " 平 局 ！ ", 870, 300);
            } else {
                g.drawString(win + " 方 获 胜！", 820, 300);
            }
        ...
        // 绘制当前走子
        if (!allx.isEmpty()){  // 若棋盘上有棋子
            int i = allx.peek();  // 获取最新棋子坐标
            int j = ally.peek();
    ...
    public class ButtonPress implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            Object obj = e.getSource();  // 判断触发哪个按钮
            if(obj == undo) {
                Undo();
            } else if(obj == concede) {
                Concede();
    ...
    public void Concede() {
        ...
        // 确认提示
        int res = JOptionPane.showConfirmDialog(this,"确定要认输吗？");
        if (res==0){
            finish = true;
            win = (isBlack==true ? "白" : "黑");
            this.repaint();
